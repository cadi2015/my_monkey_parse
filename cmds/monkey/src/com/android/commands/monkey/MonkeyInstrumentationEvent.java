...
public class MonkeyInstrumentationEvent extends MonkeyEvent {
    ...
    public int injectEvent(IWindowManager iwm, IActivityManager iam, int verbose) {
        ...
        if (cn == null || mTestCaseName == null) //检查没有获取到ComponentName或者没有获取到mTestCaseName，抛出IllegalArgumentException
            throw new IllegalArgumentException("Bad component name");

        Bundle args = new Bundle(); //创建Bundle对象
        args.putString("class", mTestCaseName); //将用例名字作为Value，key是"class"
        try {
            iam.startInstrumentation(cn, null, 0, args, null, null, 0, null); //调用AMS的startInstrumentation（）方法，这个事件莫非可以启动UiAutomator？
        } catch (RemoteException e) { //如果AMS系统服务发生错误
            Logger.err.println("** Failed talking with activity manager!"); //AMS系统服务出现问题，说明注入事件失败，
            return MonkeyEvent.INJECT_ERROR_REMOTE_EXCEPTION; //返回注入事件有错误
        }
